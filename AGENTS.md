...
- **当前阶段**：P0 完成，P1 完成，P2 完成（161 测试 / ruff+mypy clean / CI 全绿）
...
- 改动前先跑测试，确保全部用例通过（截至 P2 完成为 161 个）
- 改动后 push 远端（用户明确允许时）
...
- **P0**（完成）：项目骨架 + SQLite 建表 + 初始脚本 + 75 测试 / 97% 覆盖
- **P1**（完成）：浏览筛选 + 题目详情（HTMX 无刷新）— 见 `docs/superpowers/specs/2026-07-13-p1-browse-detail-design.md`
- **P2**（完成）：组卷 + 答案模式 + 导出 HTML/LaTeX — 见 `docs/superpowers/specs/2026-07-13-p2-composition-design.md`
- **P3**（下一个）：统计仪表盘 + 高频考点排行
- **P4-P7**：见 `PLAN.md` 第 305-313 行
